...
//  회원 등록
	...
	//  회원정보 수정
	  ...
	  public boolean isRightUser(MemberVO memberVO) {
			boolean isLoginSuccess=false;
			String email=memberVO.getEmail();
			String pwd=memberVO.getPwd();
			try {
				conn=dataFactory.getConnection();
				String query="SELECT email FROM memtbl WHERE email=? AND pwd=?";
				pstmt=conn.prepareStatement(query);
				pstmt.setString(1,email);
				pstmt.setString(2,pwd);
				ResultSet rs=pstmt.executeQuery();
	...
	public String findPwd(MemberVO memberVO) {
		...
		try {
			conn=dataFactory.getConnection();
			String query="select pwd from memtbl where name=? and email=? and phone=? ";
			pstmt=conn.prepareStatement(query);
	        pstmt.setString(1, name);
