...
public class BoardController extends HttpServlet {
	...
	private void doHandle(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (action == null || action.equals("/qna.do")) {// 어떤 매핑정보도 날라오지 않은경우
				// 오늘 코드 추가한부분
				...
				nextPage = "/qna/qna.jsp";
			} else if (action.equals("/write.do")) {
				nextPage = "/qna/write.jsp";
			} else if (action.equals("/qnaaddArticle.do")) {
				int articleNo = 0;
				...
				out.print("location.href='" + request.getContextPath() + "/qnalist/qna.do';");
				out.print("</script>");
				return;
			} else if (action.equals("/qnaView.do")) { // 글 상세창 요청
