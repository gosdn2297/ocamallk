...
public class qnaDAO {
	...
	public List selectAllArticles(Map<String, Integer> paginMap) {
		...
		try {
			...
			String query = "SELECT * FROM(SELECT ROWNUM AS recNum, articleNo,"
					...
					+ "  title, name, writeDate, views from qnatbl ORDER BY articleNo DESC))"
	...
	public List<ArticleVO> selectAllArticles() {
		...
		try {
			...
			String query = "SELECT articleNo, title, name, writeDate views FROM "
					+ "qnatbl order by articleNo desc";
			pstmt = conn.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				int articleNo = rs.getInt("articleNo");
	...
	public int selectToArticles() {
		...
		try {
			conn = dataFactory.getConnection();
			String query = "Select count(articleNo) from qnatbl";
			pstmt = conn.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
...
//         max함수를 사용해서 가장 큰 번호를 조회(+1해서 최근 글에 부여)
			String query = "select max(articleNo) from qnatbl";
			pstmt = conn.prepareStatement(query);
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
...
//   새 글 추가 메서드
	public int insertNewArticle(ArticleVO articleVO) {
		...
		try {
			...
			String query = "insert into qnatbl (articleNo, title, content, imageFileName,"
					+ " name, views) values(?, ?, ?, ?, ?, ?)";
			pstmt = conn.prepareStatement(query);
...
//   선택한 글 상세 내용 메서드
	public ArticleVO selectArticle(int articleNo) {
		...
		try {
			...
			String query = "select articleNo, title, content, name, "
					+ "writeDate, views, imageFileName from qnatbl where articleNo = ?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, articleNo);
			ResultSet rs = pstmt.executeQuery();
			rs.next();
	...
	public void updateArticle(ArticleVO article) {
		...
		try {
			conn = dataFactory.getConnection();
			String query = "update qnatbl set title=?, content=?";
...
//			}
	...
	public List<Integer> selectRemovedArticles(int articleNo) {
		...
		try {
			conn = dataFactory.getConnection();
			String query = "select articleNo from qnatbl where articleNo";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, articleNo);
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public void deleteArticle(int articleNo) {
		try {
			conn = dataFactory.getConnection();
			String query = "delete from qnatbl where articleNo=?";
			pstmt = conn.prepareStatement(query);
			pstmt.setInt(1, articleNo);
			pstmt.executeUpdate();
			pstmt.close();
